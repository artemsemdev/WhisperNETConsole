...
- The Python runtime is resolved via an `IPythonRuntime` abstraction in Core (see `docs/delivery/local-speaker-labeling/README.md`) with three implementations in preference order: (1) a relocatable bundle built with [`python-build-standalone`](https://github.com/astral-sh/python-build-standalone), the same distribution used by `uv`, `rye`, and `pdm`, pre-populated with a venv containing pyannote and its dependencies, then code-signed and notarized for macOS; (2) a managed virtualenv created during first-run setup via `ManagedVenvRuntime`, which requires Python 3.10+ on the host and installs pinned requirements on first enable; (3) system Python via `SystemPythonRuntime`, used as a dev/CI escape hatch. Option 1 eliminates user-facing Python management entirely and is the preferred long-term target, but it is **deferred**: Phase 1 ships on option 2 so the feature is not blocked on packaging. A parallel spike validates option 1 against pyannote + PyTorch; its outcome feeds into Phase 3, not Phase 0. `python-build-standalone` replaces the original PyInstaller direction because PyInstaller's PyTorch support is fragile and its notarization record on macOS is poor; `python-build-standalone` is the modern tool specifically designed for relocatable-Python-in-an-app scenarios. Without wav2vec2, the sidecar footprint is reduced by ~1 GB compared to a full WhisperX-style stack.
...
3. **Python distribution decision** — Spike to validate [`python-build-standalone`](https://github.com/astral-sh/python-build-standalone) packaging of pyannote + PyTorch into a self-contained, relocatable, notarizable bundle. If the spike fails, the fallback is the already-implemented `ManagedVenvRuntime`, which ships in Phase 1 regardless of the spike outcome. PyInstaller is not considered because its PyTorch bundling story and macOS notarization record are both unreliable.
...
- **2026-04-19 — Phase 3 shipped on `Local-Speaker-Labeling` branch, awaiting user promotion to `master`.** Phase 0 landed the preserved `WhisperToken[]` prerequisite and the sidecar contract; Phase 1 delivered `ISpeakerEnrichmentService`, `PyannoteSidecarClient`, `SpeakerMergeService`, `ManagedVenvRuntime` (default) and `SystemPythonRuntime` (escape hatch) plus the `.voxflow.json` artifact; Phase 2 delivered the Desktop Ready-screen toggle, Running-screen diarization progress band, and speaker-roster rename UI; Phase 3 sub-PRs P3.1 (CLI `--speakers` / `--no-speakers`, plus `BatchTranscribeRequest.EnableSpeakers` so batch mode mirrors the host-override pattern) and P3.2 (MCP `transcribe_file` `enableSpeakers` parameter + reflection-based schema test) close host parity. P3.3 introduces the operational runbook at [docs/runbooks/speaker-labeling.md](../runbooks/speaker-labeling.md); P3.4 adds README / ARCHITECTURE / developer-setup discoverability. `Standalone` runtime (P3.5) remains deferred pending the `python-build-standalone` spike — the `PythonRuntimeMode.Standalone` enum value is reserved but no runtime is wired to it. Promotion of `Local-Speaker-Labeling` to `master` is a user-driven decision outside the scope of this delivery.
