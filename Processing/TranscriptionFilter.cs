...
/// </summary>
internal static class TranscriptionFilter
{
    /// <summary>
    /// Filters segments for one language candidate and records skipped-segment reasons.
    /// </summary>
    public static CandidateFilteringResult FilterSegments(
        ...
        TranscriptionOptions options)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            {
                skippedSegments.Add(new SkippedSegment(segment.Start, segment.End, normalizedText, skipReason.Value));
                continue;
            }
        ...
        TranscriptionOptions options)
    {
        ...
        {
            return SegmentSkipReason.LowInformation;
        }
...
/// <summary>
/// Contains accepted and skipped segments for one filtered candidate result.
/// </summary>
internal sealed record CandidateFilteringResult(
    IReadOnlyList<FilteredSegment> AcceptedSegments,
    IReadOnlyList<SkippedSegment> SkippedSegments);

/// <summary>
/// Represents one accepted transcript segment after filtering.
/// </summary>
internal sealed record FilteredSegment(
    string Text,
    TimeSpan Start,
    TimeSpan End,
    float Probability,
...
/// <summary>
/// Represents one skipped transcript segment and the reason it was skipped.
/// </summary>
internal sealed record SkippedSegment(
    TimeSpan Start,
    TimeSpan End,
    string Text,
...
/// <summary>
/// Lists the reasons a transcript segment can be excluded from output.
/// </summary>
internal enum SegmentSkipReason
{
    EmptyText,
    NoiseMarker,
    LowProbability,
